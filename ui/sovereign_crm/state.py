...
class AppState(rx.State):
    ...
    def _create_sparkline(self, data_list, is_positive: bool) -> go.Figure:
        if not data_list:
            fig = go.Figure()
            fig.update_layout(paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', xaxis=dict(visible=False), yaxis=dict(visible=False), height=40, width=100)
            return fig
        ...
        fillcolor = 'rgba(74, 222, 128, 0.1)' if is_positive else 'rgba(248, 113, 113, 0.1)'
    ...
    def gold_candlestick_fig(self) -> go.Figure:
        ...
        if self.global_chart_type == "Líneas":
            fig.add_trace(go.Scatter(x=dates, y=closes, mode='lines', line=dict(color="#D4AF37", width=2), name="Precio"))
        elif self.global_chart_type == "Barras":
            fig.add_trace(go.Ohlc(
                x=dates, open=opens, high=highs, low=lows, close=closes,
                line=dict(width=2.5), name="Precio"
            ))
        else:
            fig.add_trace(go.Candlestick(
                x=dates, open=opens, high=highs, low=lows, close=closes, name="Precio"
            ))
        ...
        if any(v is not None for v in bb_lower):
            fig.add_trace(go.Scatter(x=dates, y=bb_lower, mode='lines', line=dict(color="rgba(0,0,0,0)", width=0), showlegend=False, hoverinfo='skip'))
        if any(v is not None for v in bb_upper):
            fig.add_trace(go.Scatter(x=dates, y=bb_upper, mode='lines', fill='tonexty', fillcolor='rgba(212, 175, 55, 0.05)', line=dict(color="rgba(212, 175, 55, 0.2)", width=1, dash="dot"), name="Banda Bollinger"))

        # Medias Móviles
        if any(v is not None for v in sma20s):
            fig.add_trace(go.Scatter(x=dates, y=sma20s, mode='lines', line=dict(color="rgba(255, 255, 255, 0.5)", width=1.5), name="SMA 20"))
        if any(v is not None for v in sma50s):
            fig.add_trace(go.Scatter(x=dates, y=sma50s, mode='lines', line=dict(color="rgba(212, 175, 55, 0.6)", width=2), name="SMA 50"))
    ...
    def oil_candlestick_fig(self) -> go.Figure:
        ...
        if self.global_chart_type == "Líneas":
            fig.add_trace(go.Scatter(x=dates, y=closes, mode='lines', line=dict(color="#B0C4DE", width=2), name="Precio"))
        elif self.global_chart_type == "Barras":
            fig.add_trace(go.Ohlc(
                x=dates, open=opens, high=highs, low=lows, close=closes,
                line=dict(width=2.5), name="Precio"
        ...
        if any(v is not None for v in bb_lower):
            fig.add_trace(go.Scatter(x=dates, y=bb_lower, mode='lines', line=dict(color="rgba(0,0,0,0)", width=0), showlegend=False, hoverinfo='skip'))
        if any(v is not None for v in bb_upper):
            fig.add_trace(go.Scatter(x=dates, y=bb_upper, mode='lines', fill='tonexty', fillcolor='rgba(176, 196, 222, 0.05)', line=dict(color="rgba(176, 196, 222, 0.2)", width=1, dash="dot"), name="Banda Bollinger"))

        # Medias Móviles
        if any(v is not None for v in sma20s):
            fig.add_trace(go.Scatter(x=dates, y=sma20s, mode='lines', line=dict(color="rgba(255, 255, 255, 0.5)", width=1.5), name="SMA 20"))
        if any(v is not None for v in sma50s):
            fig.add_trace(go.Scatter(x=dates, y=sma50s, mode='lines', line=dict(color="rgba(176, 196, 222, 0.6)", width=2), name="SMA 50"))
    ...
    async def fetch_market_quotes(self):
        ...
        try:
            async with httpx.AsyncClient(timeout=15.0) as client:
                ...
                # Gold history
                gold_hist = await client.get(f"{API_URL}/market/history/XAU?period={self.global_time_period}")
                if gold_hist.status_code == 200:
                    self.gold_historical = gold_hist.json()
                
                # Oil history
                oil_hist = await client.get(f"{API_URL}/market/history/WTI?period={self.global_time_period}")
