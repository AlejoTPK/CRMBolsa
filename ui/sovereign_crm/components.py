...
def glass_card(*children, **kwargs) -> rx.Component:
    ...
    return rx.box(*children, style=default_style, **kwargs)
...
def kpi_card(
    ...
    price_str: str,
    change_str: str,
    is_positive: rx.Var[bool],
    status: str,
    sparkline_fig: rx.Var[go.Figure],
) -> rx.Component:
    ...
    return glass_card(
        rx.hstack(
            ...
            rx.vstack(
                ...
                rx.text(
                    change_str, color=trend_color, font_weight="bold", font_size="1rem"
                ),
                spacing="1",
                align_items="start",
            ),
...
def navbar() -> rx.Component:
    ...
    return rx.hstack(
        rx.hstack(
            rx.link(
                rx.heading(
                    "Sovereign", size="6", color=COLORS["primary"], font_family="serif"
                ),
                href="/",
            ...
            rx.hstack(
                rx.link(
                    rx.hstack(rx.icon("layout-dashboard"), rx.text("Dashboard")),
                    color=rx.cond(
                        AppState.current_page == "Dashboard",
                        COLORS["text_main"],
                        COLORS["text_muted"],
                    ),
                    on_click=AppState.set_page("Dashboard"),
                    cursor="pointer",
                    _hover={"color": COLORS["text_main"]},
                ),
                rx.link(
                    rx.hstack(rx.icon("pie-chart"), rx.text("Portafolio")),
                    color=rx.cond(
                        AppState.current_page == "Portafolio",
                        COLORS["text_main"],
                        COLORS["text_muted"],
                    ),
                    on_click=AppState.set_page("Portafolio"),
                    cursor="pointer",
                    _hover={"color": COLORS["text_main"]},
                ),
                rx.link(
                    rx.hstack(rx.icon("newspaper"), rx.text("Noticias & Análisis")),
                    color=rx.cond(
                        AppState.current_page == "Noticias",
                        COLORS["text_main"],
                        COLORS["text_muted"],
                    ),
                    on_click=AppState.set_page("Noticias"),
                    cursor="pointer",
                    _hover={"color": COLORS["text_main"]},
                ),
                spacing="5",
...
def candlestick_chart(title: str, symbol: str, fig: rx.Var, color: str) -> rx.Component:
    ...
    return rx.vstack(
        ...
        rx.hstack(
            ...
            rx.hstack(
                rx.button(
                    ...
                    size="1",
                    variant=rx.cond(
                        AppState.global_time_period == "1d", "solid", "soft"
                    ),
                    color_scheme="gray",
                    on_click=AppState.change_time_period("1d"),
                    cursor="pointer",
                ),
                rx.button(
                    "1W",
                    size="1",
                    variant=rx.cond(
                        AppState.global_time_period == "5d", "solid", "soft"
                    ),
                    color_scheme="gray",
                    on_click=AppState.change_time_period("5d"),
                    cursor="pointer",
                ),
                rx.button(
                    "1M",
                    size="1",
                    variant=rx.cond(
                        AppState.global_time_period == "1mo", "solid", "soft"
                    ),
                    color_scheme="gray",
                    on_click=AppState.change_time_period("1mo"),
                    cursor="pointer",
                ),
                rx.button(
                    "3M",
                    size="1",
                    variant=rx.cond(
                        AppState.global_time_period == "3mo", "solid", "soft"
                    ),
                    color_scheme="gray",
                    on_click=AppState.change_time_period("3mo"),
                    cursor="pointer",
                ),
                rx.button(
                    "1Y",
                    size="1",
                    variant=rx.cond(
                        AppState.global_time_period == "1y", "solid", "soft"
                    ),
                    color_scheme="gray",
                    on_click=AppState.change_time_period("1y"),
                    cursor="pointer",
                ),
                spacing="1",
                bg=COLORS["surface_high"],
        ...
        rx.box(
            rx.plotly(
                data=fig, height="320px", width="100%", config={"displayModeBar": False}
            ),
            width="100%",
            bg=COLORS["surface"],
            border_radius="12px",
